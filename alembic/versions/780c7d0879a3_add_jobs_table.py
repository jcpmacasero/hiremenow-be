...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE workmode AS ENUM ('onsite', 'accommodation_provided')")
    op.execute("CREATE TYPE jobtype AS ENUM ('full_time', 'part_time', 'contract', 'seasonal')")
    op.execute("CREATE TYPE jobstatus AS ENUM ('draft', 'pending_approval', 'active', 'paused', 'closed', 'rejected')")
    op.execute("CREATE TYPE salaryperiod AS ENUM ('hourly', 'monthly')")
    ...
    op.create_table(
        ...
        sa.Column('work_mode', postgresql.ENUM('onsite', 'accommodation_provided', name='workmode', create_type=False), nullable=False, server_default='onsite'),
        sa.Column('job_type', postgresql.ENUM('full_time', 'part_time', 'contract', 'seasonal', name='jobtype', create_type=False), nullable=False, server_default='full_time'),
        ...
        sa.Column('salary_period', postgresql.ENUM('hourly', 'monthly', name='salaryperiod', create_type=False), nullable=False, server_default='monthly'),
        ...
        sa.Column('status', postgresql.ENUM('draft', 'pending_approval', 'active', 'paused', 'closed', 'rejected', name='jobstatus', create_type=False), nullable=False, server_default='draft'),
